...
class EducationCertification(models.Model):
    ...
    code = fields.Char(
        string="Code", required=True, default=lambda self: _("New"), readonly=True
    )

    type = fields.Selection(
        [("subject", "Subject"), ("course", "Course")], string="Type"
    )
    ...
    record_subject_id = fields.Many2one(
        comodel_name="education.record.subject", string="Record Subject"
    )
    ...
    def _change_course_id(self):
        ...
        subject_fields_domain = [("id", "in", self.course_id.subject_ids.ids)]
