...
class EducationGroup(models.Model):
    ...
    date_from = fields.Date(
        string="From Date", readonly=True, states={"draft": [("readonly", False)]}
    )
    date_to = fields.Date(
        string="To Date", readonly=True, states={"draft": [("readonly", False)]}
    )
        ...
        for val in vals:
            if val.get("code", _("New")) == _("New"):
                if "company_id" in val:
                    val["code"] = self.env["ir.sequence"].with_context(
                        force_company=val["company_id"]
                    ).next_by_code("education.group") or _("New")
                else:
