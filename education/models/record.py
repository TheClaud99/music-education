    ...
    def create(self, vals):
        for val in vals:
            if val.get("code", "New") == "New":
                val["code"] = (
                    self.env["ir.sequence"].next_by_code("education.record") or "New"
                )
    ...
    def _compute_last_record_subject_group_id(self):
        for record in self:
            record.last_record_subject_group_id = (
                record.record_subject_group_ids and record.record_subject_group_ids[-1]
            )
...
class EducationRecordSubjectGroup(models.Model):
    ...
    record_subject_id = fields.Many2one(
        comodel_name="education.record.subject", string="Subject Record"
    )
