...
class EducationExam(models.Model):
    ...
    result_ids = fields.One2many(
        comodel_name="education.result", inverse_name="exam_id", string="Results"
    )

    date = fields.Date(string="Date", required=True)

    grading_id = fields.Many2one(
        comodel_name="education.grading.scale", string="Grading", required=True
    )
    ...
    def _change_group_id(self):
        ...
        subject_fields_domain = [
            ("id", "in", self.group_id.course_id.subject_ids.mapped("subject_id").ids)
        ]
    ...
    def set_planned(self):
        ...
        self.state = "planned"
        values = []
        for record_subject_group in (
            self.group_id.enrollment_ids.mapped("record_id.record_subject_ids")
            .filtered(lambda r: r.subject_id == self.subject_id)
            .mapped("record_subject_group_ids")
        ):
            values.append(
                (
                    0,
    ...
    def unlink(self):
        for record in self:
            if record.state not in ["done"]:
                ...
                raise ValidationError(_("You only can delete draft and planned exams"))
