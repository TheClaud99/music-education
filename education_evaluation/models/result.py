...
class EducationEvaluable(models.AbstractModel):
    ...
    score_type = fields.Selection(
        [("manual", "Manual"), ("computed", "Computed")],
        default="computed",
        string="Score type",
        required=True,
    )
    ...
    score_computed = fields.Float(
        string="Computed Score", compute="_compute_score_computed"
    )
    score_manual = fields.Float(string="Score")
    grading_id = fields.Many2one(
        string="Grading", comodel_name="education.grading.scale", readonly=True
    )
    grade_id = fields.Many2one(
        string="Grade", comodel_name="education.grade", compute="_compute_grade"
    )
    ...
    def _compute_score(self):
        for evaluable in self:
            ...
            evaluable.score = (
                evaluable.score_type == "computed"
                and evaluable.score_computed
...
class EducationResult(models.Model):
    ...
    exam_id = fields.Many2one(comodel_name="education.exam", string="Exam")

    score = fields.Float(string="Score")

    record_subject_group_id = fields.Many2one(
        comodel_name="education.record.subject.group", string="Subject Record Group"
    )

    student_id = fields.Many2one(
        comodel_name="res.partner",
        string="Student",
        related="record_subject_group_id.record_subject_id." "record_id.student_id",
    )

    weight = fields.Float(string="Weight", related="exam_id.weight", readonly=True)
...
class EducationRecordSubject(models.Model):
    ...
    score = fields.Float(
        string="Score", related="last_record_subject_group_id.score", readonly=True
    )
    grade_id = fields.Many2one(
        string="Grade",
...
class EducationRecordSubjectGroup(models.Model):
    ...
    def _compute_score_computed(self):
        for record in self:
            ...
            record.score_computed = sum(
                exams.mapped(lambda e: e.exam_id.weight * e.score)
            ) / (total_weight or 1)
...
class EducationRecord(models.Model):
    ...
    def _compute_score_computed(self):
        for record in self:
            ...
            record.score_computed = sum(
                subjects.mapped(lambda e: e.weight * e.score)
            ) / (total_weight or 1)
