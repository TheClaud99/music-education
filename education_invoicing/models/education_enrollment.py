...
class EducationEnrollment(models.Model):
    ...
    invoices_count = fields.Integer(
        string="Invoices", compute="_compute_count_invoices"
    )
    ...
    invoicing_price = fields.Float(
        string="Invoicing Price", related="invoicing_method_id.amount"
    )
    invoicing_method_id = fields.Many2one(
        comodel_name="education.invoicing.method", string="Invoicing Method"
    )
    invoicing_method_amount = fields.Float(
        string="Invoicing Method Amount", compute="_compute_invoicing_method_amount"
    )
    ...
    first_fee_date = fields.Date(string="First Fee Date")
    ...
    def compute_invoicing_method(self):
        ...
        date = self.first_fee_date or self.group_id.date_from or fields.Date.today()
            ...
            for a in range(0, line.quantity):
                if a >= 0:
                    if line.recurring_rule_type == "monthly":
                        last_date = fields.Date.from_string(line_date) + relativedelta(
                            months=a
                        )
                ...
                if self.course_id.pack:
                    ...
                    invoice_line_name = (
                        self.course_id.name + "(" + self.group_id.name + ")"
                    )
                invoice_line_data = {
                    "product_id": product_id.id,
                    "name": invoice_line_name,
                    "account_id": account_journal_id.default_debit_account_id.id,
                    "quantity": 1,
...
class EducationEnrollmentInvoicingMethodLine(models.Model):
    ...
    name = fields.Selection(
        [("enrollment", "Enrollment"), ("fee", "Fee"), ("refund", "Refund")],
        string="Type",
    )
